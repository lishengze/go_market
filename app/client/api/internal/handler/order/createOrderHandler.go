	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "POST" {
			...
			if len(iv) == 0 {
				...
				l := order.NewCreateOrderLogic(r, r.Context(), svcCtx)
		...
		if r.Method == "GET" {
			iv := r.Header.Get("X-Encrypt-Iv")
			logx.Infof("CreateOrderLogic iv:%s", iv)
