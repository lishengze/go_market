...
func AddTestHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "POST" {
			...
			if len(iv) == 0 {
				...
				l := user.NewAddTestLogic(r, r.Context(), svcCtx)
				resp, err := l.AddTest(&req)
		...
		if r.Method == "GET" {
			iv := r.Header.Get("X-Encrypt-Iv")
			logx.Infof("AddTestLogic iv:%s", iv)
