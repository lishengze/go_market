	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "POST" {
			...
			if len(iv) == 0 {
				...
				l := user.NewGetAccountInfoLogic(r, r.Context(), svcCtx)
			...
			if err := json.Unmarshal(requestPlainMsg, &req); err != nil {
				logx.Errorf("GetAccountInfoLogic Unmarshal err:%+v", err)
				result.ParamErrorResult(r, w, err, svcCtx.PemFileBase.AESKey, svcCtx.Config.ResponseEncrypted)
				return
			}

			logx.Infof("GetAccountInfoLogic req:%+v", req)
		...
		if r.Method == "GET" {
			iv := r.Header.Get("X-Encrypt-Iv")
			logx.Infof("GetAccountInfoLogic iv:%s", iv)
