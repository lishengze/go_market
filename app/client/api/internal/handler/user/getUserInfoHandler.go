	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "POST" {
			...
			if len(iv) == 0 {
				...
				l := user.NewGetUserInfoLogic(r, r.Context(), svcCtx)
		...
		if r.Method == "GET" {
			iv := r.Header.Get("X-Encrypt-Iv")
			logx.Infof("GetUserInfoLogic iv:%s", iv)
